  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'employee', 'team', 'customer'],
      }),
  );
              ...
              {hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/employees/create?organization_id=${data?.id}`}>
                      ...
                      <Tbody>
                        {data?.employee?.map((record) => (
                          ...
                          >
                            ...
                            <Td>{record.performance_evaluation}</Td>
                            <Td>
              ...
              {hasAccess('team', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/teams/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.team?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/teams/view/${record.id}`)} key={record.id}>
                            <Td>
                              {hasAccess('team', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/teams/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
